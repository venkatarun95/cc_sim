...
static u32 bbr_packets_in_net_at_edt(struct sock *sk, u32 inflight_now)
{
	...
	interval_us = ((u64) (edt_ns - now_ns)) / NSEC_PER_USEC;
...
static void bbr_reset_lt_bw_sampling_interval(struct sock *sk)
{
	...
	bbr->lt_last_stamp = (u64)(tp->delivered_mstamp / USEC_PER_MSEC);
...
static void bbr_lt_bw_sampling(struct sock *sk, const struct rate_sample *rs)
{
	...
	t = (u64)(tp->delivered_mstamp / USEC_PER_MSEC) - bbr->lt_last_stamp;
	if ((s32)t < 1)
...
static void bbr_update_bw(struct sock *sk, const struct rate_sample *rs)
{
	 ...
	 */
	bw = ((u64)rs->delivered * BW_UNIT / rs->interval_us);
	...
	if (!rs->is_app_limited || bw >= bbr_max_bw(sk)) {
		// printf("Updating with bw = %lld vs bbr_max_bw(sk) = %lld\n", bw, bbr_max_bw(sk));
...
static void bbr_check_probe_rtt_done(struct sock *sk)
{
	...
	if (!(bbr->probe_rtt_done_stamp &&
	      after(tcp_jiffies32(sk), bbr->probe_rtt_done_stamp)))
		return;


	bbr->min_rtt_stamp = tcp_jiffies32(sk);  /* wait a while until PROBE_RTT */
...
static void bbr_update_min_rtt(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (bbr->mode == BBR_PROBE_RTT) {
		...
		if (!bbr->probe_rtt_done_stamp &&
			...
			bbr->probe_rtt_done_stamp = tcp_jiffies32(sk) +
...
static void bbr_init(struct sock *sk)
{
	...
	__sync_bool_compare_and_swap(&sk->sk_pacing_status, SK_PACING_NONE, SK_PACING_NEEDED);
}
